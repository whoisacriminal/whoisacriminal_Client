...
export default function SelectEvidence() {
  ...
  const prev = () => setIndex((currentIndex) => Math.max(currentIndex - 1, 0))
  const next = () => {
  ...
  const onTouchEnd = (event) => {
    ...
    const dx = event.changedTouches[0].clientX - dragStart.current
    if (dx < -50) next()
    else if (dx > 50) prev()
  ...
  const onMouseMove = () => {
    if (dragStart.current !== null) dragging.current = true
  }
    ...
    return (
      <div className="se-shell">
        ...
        <div className="se-fallback">
          <p>인물 정보가 없습니다.</p>
          <button onClick={goToSelect}>돌아가기</button>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="se-shell">
      ...
      <button className="se-back-btn" onClick={goToSelect} aria-label="선택 화면으로 돌아가기">
        &lt;
      </button>

      <button className="se-next-btn" onClick={() => navigate(NEXT_ROUTE)} aria-label="심문 선택으로 이동">
        다음
      </button>
      ...
      {evidences.length > 0 ? (
        <div className="se-carousel-wrap">
          ...
          >
            {evidences.map((evidence, slideIndex) => (
              <div className={getSlideClassName(slideIndex)} key={evidence.src}>
                <img
                  src={evidence.src}
                  alt={`${selectedCharacter.name} 증거 ${slideIndex + 1}`}
                  draggable="false"
                />
              </div>
            ))}
          ...
          {index > 0 && (
            <button className="se-arrow se-arrow--left" onClick={prev} aria-label="이전 증거">
              &lt;
            </button>
          )}
          <button className="se-arrow se-arrow--right" onClick={next} aria-label="다음 증거">
            &gt;
          </button>

          {evidences.length > 1 && (
            <div className="se-dots">
              {evidences.map((evidence, dotIndex) => (
                <button
                  key={evidence.src}
                  className={`se-dot${dotIndex === index ? ' se-dot--active' : ''}`}
                  onClick={() => setIndex(dotIndex)}
                  aria-label={`${dotIndex + 1}번째 증거`}
                />
              ))}
            </div>
          )}
